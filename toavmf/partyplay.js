...
$(document).ready(function() {
		...
		navn.mouseup(function() { 	SDDn.css("background-image","url(../images/party/Header/header_menu_noteov.png)");
													location.href = 'http://funsumer.net/note';
												});
	...
	$.each($("#info_memberpage > .header > .img"),function() {
	...
	$.each($("#info_anouncepage > .header > .img"),function() {
		...
		$(this).mouseup(function(){
			...
			if($.browser.msie && $.browser.version == "8.0") {
				...
				if ( party_trans_defence == 0 ) {
						...
						$("#container").css("visibility","hidden");	// container hidden
					...
					if ( party_trans_defence == 0 ) {
							...
							loading_circuling = setInterval("unlimited()",25);
							$("#container").fadeTo(200,0);	// container hidden
